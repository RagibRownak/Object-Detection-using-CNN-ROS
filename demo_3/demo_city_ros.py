...
class PanopticSegmentationNode(Node):
    ...
    def __init__(self, config_file, weights, confidence_threshold=0.5, 
        ...
        self.adaptive_resize = True  # Set to False if you want to use a fixed size
        self.fixed_size = (640, 384)  # Fixed size (must be multiples of 64x32)
    ...
    def preprocess_image_for_panoptic_deeplab(self, image):
        """Resize image to the exact dimensions required by the Cityscapes model"""
        # For Cityscapes models, we MUST use exactly 1024x512 dimensions
        # This is critical for the model's pooling operations to work correctly
        target_w, target_h = 1024, 512
        
        # Log the resize operation
        h, w = image.shape[:2]
        self.get_logger().info(f"Resizing from {w}x{h} to {target_w}x{target_h}")
        
        # Resize the image to exactly the dimensions needed
    ...
    def process_images(self):
        ...
        while self.running and rclpy.ok():
            ...
            try:
                # Add preprocessing step for Panoptic-DeepLab
                resized_color = self.preprocess_image_for_panoptic_deeplab(color_copy)
                
                # CRITICAL: Resize to EXACTLY 1024x512 as required by the model
                processed_frame = cv2.resize(resized_color, (1024, 512))
                
                # Process with detectron2 using the resized image
                start_time = time.time()
                with autocast("cuda"):
                    predictions, visualized_output = self.demo.run_on_image(processed_frame)
                ...
                if depth_available and "instances" in predictions:
                    # Create a resized depth image that matches the processed color image dimensions
                    with self.depth_lock:
                        depth_img = self.latest_depth_image
                        if depth_img is not None:
                            # Resize depth to match the processed color dimensions
                            h, w = resized_color.shape[:2]
                            resized_depth = cv2.resize(depth_img, (w, h), 
                                                     interpolation=cv2.INTER_NEAREST)
                        else:
                            resized_depth = None

                    if resized_depth is not None:
                        # Use resized_depth instead of depth_img
                        ...
                        if len(instances) > 0:
                            ...
                            for idx in range(len(instances)):
                                if instances.has("pred_boxes"):
                                    ...
                                    x1 = max(0, min(x1, resized_depth.shape[1]-1))
                                    x2 = max(0, min(x2, resized_depth.shape[1]-1))
                                    y1 = max(0, min(y1, resized_depth.shape[0]-1))
                                    y2 = max(0, min(y2, resized_depth.shape[0]-1))
                                    ...
                                    # Crop depth region
                                    depth_crop = resized_depth[y1:y2, x1:x2]
    ...
    def apply_segmentation_to_pointcloud(self, pointcloud, segmentation_data):
        ...
        try:
            ...
            # Handle DeepLab format which doesn't provide segment info
            if segments_info is None:
                self.get_logger().info("DeepLab panoptic segmentation: Creating segments in apply_segmentation")
                segments_info = []
                # Create synthetic segment info from unique IDs in the segmentation
                unique_ids = np.unique(panoptic_seg)
                for id in unique_ids:
                    if id == 0:  # Skip background
                        continue
                    # Create a segment entry for each unique ID
                    segments_info.append({
                        'id': int(id),
                        'category_id': int(id % 20) + 1,  # Create varied colors
                        'isthing': id > 1000  # Common convention: things have IDs > 1000
                    })
        ...
        try:
            ...
            if has_new_data:
                ...
                if 'panoptic_seg' in current_seg_data['predictions']:
                    panoptic_data = current_seg_data['predictions']['panoptic_seg']
                    
                    # For DeepLab models, handle the different segmentation format
                    if isinstance(panoptic_data, tuple) and len(panoptic_data) >= 2:
                        # Traditional format [tensor, segments_info]
                        panoptic_seg = panoptic_data[0].cpu().numpy()
                        segments_info = panoptic_data[1]
                        if segments_info is None:
                            # Handle DeepLab format which doesn't provide segment info
                            self.get_logger().info("DeepLab panoptic segmentation: Creating segments from mask")
                            segments_info = []
                            # Create synthetic segment info from unique IDs in the segmentation
                            unique_ids = np.unique(panoptic_seg)
                            for id in unique_ids:
                        ...
                        for id in unique_ids:
                            ...
                            segments_info.append({
                                'id': int(id),
                                'category_id': int(id % 20) + 1,
                                'isthing': id > 1000
                            })
                    
                    # If we still have no segments, log and exit
                    if not segments_info:
                        self.get_logger().warn("No segments found in panoptic segmentation")
                        return False
                    ...
                    # Apply segmentation to pointcloud
                    segmented_cloud = self.apply_segmentation_to_pointcloud(current_cloud, current_seg_data)
                    
                    # Publish the segmented cloud
                    self.points_publisher.publish(segmented_cloud)
                    
                    # Cache for reuse
                    with self.points_lock:
                        self.last_segmented_cloud = segmented_cloud
                    
                    self.get_logger().info("Published segmented pointcloud with Cityscapes colors")
                    return True
    ...
    def is_obstacle_class(self, category_id, metadata):
        ...
        # Cityscapes classes to ignore (not obstacles)
        ignore_classes = [
            "sky", "road", "sidewalk", "terrain", "ground", "floor", "path"
        ]
